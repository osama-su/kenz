...
@section('page_header')
    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
        <li class="breadcrumb-item">
            <a href="{{route('dashboard.expense-types.index')}}" class="text-muted">المصروفات</a>
        </li>
    </ul>
@endsection
@section('content')
    <div class="card card-custom">
        <div class="card-header py-3">
            ...
            <div class="card-toolbar">
                <!--begin::Button-->
                @can('create_product')
                    <a href="{{route('dashboard.expense-types.create')}}" class="btn btn-primary font-weight-bolder">
                ...
                <table class="table table-separate table-head-custom table-checkable" id="kt_datatable1">
                    ...
                    @foreach($expenseTypes as $expense)
                        <tr id="row-{{ $expense->id }}">
                            ...
                            <td>
                                @can('update_product')
                                    <a href="{{route('dashboard.expense-types.edit', ['expense_type' => $expense->id])}}"
                                       class="btn btn-sm btn-clean btn-icon btn-icon-md">
                                        <i class="la la-edit"></i>
                                    </a>
                                @endcan
                                @can('delete_product')
                                    <a data-url="{{ route('dashboard.expense-types.destroy',['expense_type' => $expense->id]) }}"
